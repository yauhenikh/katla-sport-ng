...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(s => hiveSection.isDeleted = true);
  }

  onUndelete(hiveSectionId: number) {
    var hiveSection = this.hiveSections.find(s => s.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(s => hiveSection.isDeleted = false);
  }
}
